...
- **Sheet CẦN CUỘN nội dung dài → KHÔNG dùng heroui `BottomSheet`, dùng gorhom `BottomSheetModal` trực tiếp.** Lý do: heroui `BottomSheet.Content` LUÔN bọc children trong gorhom `BottomSheetView`, mà `BottomSheetView` khi mount đăng ký scrollable `type=VIEW` (effect cha chạy SAU effect con) → ghi đè đăng ký `SCROLLVIEW` của `BottomSheetScrollView` → mọi pan kéo sheet, KHÔNG cuộn (fix type sau bằng focusHook cũng vô ích vì gesture đã `.simultaneousWithExternalGesture` lúc mount với active=VIEW). Các sheet heroui hiện có chưa lộ lỗi chỉ vì nội dung chưa đủ dài để tràn. Cách đúng: `BottomSheetModal` + `BottomSheetModalProvider` (đã thêm ở `_layout.tsx` trong HeroUINativeProvider), `BottomSheetScrollView` làm con TRỰC TIẾP (modal portal lên trên → z-order đúng; gorhom `BottomSheet` non-modal render inline → bị nội dung trang đè). Điều khiển `ref.present()/dismiss()` theo `isOpen` + `onDismiss`, backdrop `BottomSheetBackdrop` pressBehavior="close", theme qua `backgroundStyle`/`handleIndicatorStyle`. Reference: [MyBalanceExplanationSheet.tsx](src/components/trip/MyBalanceExplanationSheet.tsx). Sheet ngắn không tràn → giữ heroui `BottomSheet` cho nhất quán. Memory: [feedback_heroui_bottomsheet_scroll.md](C:/Users/tranh/.claude/projects/c--Users-tranh-fair-pay/memory/feedback_heroui_bottomsheet_scroll.md).
...
- **`android/` bị GITIGNORE** (`.gitignore` dòng 50, `git ls-files android/` = 0) — KHÔNG commit vào repo; do `expo prebuild` sinh ra từ `app.json`. Fresh clone / máy mới PHẢI chạy `npx expo prebuild --platform android` trước khi build. Hệ quả: KHÔNG có lưới git cho `android/` (không `git diff android/` để bắt prebuild làm hỏng config) → mọi hand-edit trong `android/` (signing `fairpay`, `versionCode`, dòng `firebase-analytics`) phải TỰ verify + restore sau mỗi prebuild (kể cả merge mode, KHÔNG chỉ `--clean`; verified: merge giữ signing/analytics nhưng reset `versionCode` → 1). Build local: `cd android && ./gradlew bundleRelease`. Widget native (`react-native-android-widget`) cũng do prebuild sinh từ plugin trong `app.json` + entry `index.js` → không có file native nào phải commit tay. (Verified 2026-07-15; câu cũ "commit android/ KHÔNG ignore" là SAI — [memory](C:/Users/ADMIN/.claude/projects/d--fair-pay/memory/project_android_gitignored.md).)
...
- Verify sau prebuild --clean: `android/` gitignore nên KHÔNG dùng `git diff android/` được — phải TỰ đối chiếu (checklist trên: signing `fairpay` + `firebase-analytics` + `versionCode` + config sinh từ app.json còn đủ), hoặc build thử. Nếu app.json bị edit nhầm (mất `googleServicesFile` hoặc plugin `expo-notifications` hoặc `AD_ID` trong blockedPermissions) → prebuild sẽ KHÔNG sinh config tương ứng → restore app.json + prebuild lại. Memory chi tiết: [feedback_fcm_prebuild_overwrite.md](C:/Users/ADMIN/.claude/projects/d--fair-pay/memory/feedback_fcm_prebuild_overwrite.md).
...
- **Fair Pay pin chặt** trong [package.json](package.json):
  - `dependencies`: 4 dep critical bỏ caret, dùng exact version (`"uniwind": "1.5.0"`, `"react-native-reanimated": "4.2.3"`, `"react-native-worklets": "0.7.2"`, `"tailwindcss": "4.2.2"`).
  - Reanimated pin `4.2.3` (KHÔNG hạ về 4.2.1): 4.2.3 chứa fix "commits during the Android draw pass" (PR #9123, cherry-pick #9072) — sửa crash native `IllegalStateException: <ViewGroup> contains null child at index N ... dispatchGetDisplayList` trên OPPO/Realme (Android 16 throw message này; Android ≤15 là NPE `mViewFlags`) khi navigate lúc animation Reanimated đang chạy (vd pin nhanh home → trip detail). 4.2.3 vẫn nhận worklets 0.7.x (`compatibility.json`: `0.7.x | 0.8.x`) — KHÔNG cần bump worklets như 4.3.x.
